...
class Detail_base extends CI_Model{
    ...
    public function _moneyConvert(&$var,$type ="SGD"){
        if(is_numeric($var))
        {
            //convert to 4 digit
            switch($type){
                case 'IDR':
                    ...
                    break;
                default:
                    return $var = number_format((float)$var, 2, '.', ',');
            }
    ...
    public function _date_abb($date){

        $_date  = "";
        $nat_date =  date('Y-m-d', strtotime($date));
        $date_arr = explode("-" ,$nat_date);
        $y = $date_arr[0];
        $m = $date_arr[1];
        $d = $date_arr[2];
